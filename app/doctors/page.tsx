...
export default function DoctorsPage() {
  ...
  const filteredRecords = useMemo(() => {
    return mockDoctorData.filter((record) => {
      // 医師名/医療機関名フィルター
      if (
        selectedName &&
        !record.name.includes(selectedName) &&
        !record.hospital.includes(selectedName)
      ) {
        return false;
      }
    ...
    <MainLayout>
      <div className="p-4 md:p-6">
        ...
        <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">医師管理</h1>
          </div>
            ...
            <div key={record.id} className="bg-white rounded-lg shadow p-4">
              <div className="mb-3 pb-3 border-b border-gray-200">
                <div className="flex justify-between items-start">
                  ...
                  <button
                    onClick={() => router.push(`/doctors/${record.id}/edit`)}
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between py-2 border-b border-gray-100">
                  <span className="text-sm text-gray-600">個別携帯番号</span>
                  <span className="text-sm font-medium text-gray-900">
                    {record.mobilePhone}
                  </span>
                </div>
