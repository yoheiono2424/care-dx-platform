...
export default function PharmaciesPage() {
  ...
  const filteredRecords = useMemo(() => {
    return mockPharmacyData.filter((record) => {
      // 薬局名フィルター
    ...
    <MainLayout>
      <div className="p-4 md:p-6">
        ...
        <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">薬局管理</h1>
          </div>
          <div className="flex gap-2">
            <Button
              variant="secondary"
              onClick={() => router.push('/pharmacies/create')}
            >
              薬局作成
            </Button>
          </div>
            ...
            <div key={record.id} className="bg-white rounded-lg shadow p-4">
              <div className="mb-3 pb-3 border-b border-gray-200">
                <div className="flex justify-between items-start">
                  ...
                  <button
                    onClick={() => router.push(`/pharmacies/${record.id}/edit`)}
              ...
              {filteredRecords.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-center">
                    <button
                      onClick={() =>
                        router.push(`/pharmacies/${record.id}/edit`)
                      }
